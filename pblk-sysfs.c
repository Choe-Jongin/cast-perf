...
static ssize_t pblk_sysfs_get_cast_latency(struct pblk *pblk, char *page)
{
	ssize_t sz = 0;
	int i;

	for (i = 0; i < MAX_LATENCY_CHUNK; i++) {
		sz += snprintf(page + sz, PAGE_SIZE - sz,"%d ", pblk->c_perf->latency[i]);
	}
	sz += snprintf(page + sz, PAGE_SIZE - sz,"\n");
	return sz;
}
...
static ssize_t pblk_sysfs_cast_active(struct pblk *pblk, const char *page,
				   size_t len)
{
	size_t c_len;
	int active;
	...
	if (kstrtouint(page, 0, &active))
		return -EINVAL;

	pblk->c_perf->mgr->set_active(pblk->c_perf->mgr, active);
...
static struct attribute sys_cast_latency = {
	.name = "latency",
	.mode = 0644,
};
static struct attribute sys_cast_active = {
	.name = "cast_active",
	.mode = 0200,
};
			       ...
			       char *buf)
{
	...
	else if (strcmp(attr->name, "latency") == 0)
		return pblk_sysfs_get_cast_latency(pblk, buf);
#ifdef CONFIG_NVM_PBLK_DEBUG
				...
				const char *buf, size_t len)
{
	...
	if (strcmp(attr->name, "cast_active") == 0)
		return pblk_sysfs_cast_active(pblk, buf, len);
	else if (strcmp(attr->name, "max_sec_per_write") == 0)
